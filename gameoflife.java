...
public class gameoflife extends JFrame implements Runnable {
	...
	private void nextStep() {
	    ...
	    for (int x = 0; x < xSize; x++) {
	        for (int y = 0; y < ySize; y++) {
	            ...
	            //checks if a dead cell has exactly 3 neighbors, and sets the color to whichever user accounts for more of those three cells
	            if (grid[x][y] == 0 && neighbors.size() == 3) newGrid[x][y] = oneCount > twoCount ? 1 : 2;
	            //checks if a lives cell has too few or too many neighbors and sets it dead or alive accordingly
	...
	private void cropGrid() {
	    ...
	    if (firstRow != -1) {
	    	//creates temp grid of the specified boundary and fills with the cells
	...
	private void writeToFile(int[][] cgrid, int xSize, int ySize) {
        ...
        if (willSave == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                //saves first two lines with name and a new line, each preceded with a '!'
            ...
            //add the newly saved shape to the right-click shape menu for easy access
            shapeMenu.add(new ShapeMenuItem(currentFile));
        }
	}

	//open file and add it to the right-click shape menu to insert into the grid
    ...
    public void run() {
        ...
        p.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
            	//gets current location on the panel and divides it by the size of the panel and size of the grid to get the box number
            	//uses BigDecimal class because I couldn't find another way to make sure that the number calculated was rounded down
            	//the number needs to be rounded down so that the correct box is chosen and filled in
                ...
                //if the click is a right click, don't toggle cell state, but open shape menu
                if (SwingUtilities.isRightMouseButton(e))
                    shapeMenu.show(e.getComponent(), e.getX(), e.getY());
                //if it isn't a right click, and the grid is edittable, toggle the cell state and repaint
                else if (edit) {
        ...
        //set the default close operation to fully close the program when the user presses the 'x' in the corner
        ...
        private static final long serialVersionUID = -897367256144487579L;
		    ...
		    for (int x = 0; x < xSize; x++) {
		        for (int y = 0; y < ySize; y++) {
		            ...
		            //fills the rectangle, sets the color to black and then draws the grid
		            g2.fill(r2d);
	...
	class SizeMenuItem extends JMenuItem {
        ...
        public SizeMenuItem(int _x, int _y) {
            ...
            x = _x;
            y = _y;
            //adds action listener to resize grid to the correct dimensions and repaint
	...
	class SpeedMenuItem extends JMenuItem {
        ...
        public SpeedMenuItem(int _speed) {
        	//initializes name
	        super (_speed + "ms");
	        //adds action listener to change the timer speed to speed up animation
	...
	class ShapeMenuItem extends JMenuItem {
	    ...
	    public ShapeMenuItem(File _shapeFile) {
	        ...
	        try {
	            ...
	            reader.readLine();
	            reader.readLine();
	            //sets a mark so that we can reset back to this point after getting shape width
	            reader.mark(10000);
	            //get shape size to create the grid by counting lines and characters on each line
	            ...
	            reader.reset();
	            //loops through each line of the shape and converts the characters to cell states
	        ...
	        //catch file issues while parsing, and prompt file error to the user
	        } catch (IOException e) {
	            sGrid = new int[0][0];
	        ...
	        //adds the action listener to put the file into the current grid and repaint
	        addActionListener(e -> {
	            try {
	                for (int x = 0; x < xSize; x++)
	                    for (int y = 0; y < ySize; y++)
	...
	//file filter class to filter out non-lexicon files in the file choosers used in the program
